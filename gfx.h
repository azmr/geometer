...
RasterCircleLine(memory_arena *Arena, f32 Radius)
{
	...
	Result.Memory = PushSize(Arena, Result.Width * Result.Height);
...
#define DRAW_PIXEL(x, y) Pixel = Result.Memory + GetLinearizedPosition(Result.Width, V2Add(Centre, V2(x, y)));\
		*Pixel = 255;
		...
		CandidateH.X -= 1.f;
		CandidateV.Y -= 1.f;
		f32 DistSqH = LenSq(V2Sub(CandidateH, Centre));
		f32 DistSqV = LenSq(V2Sub(CandidateV, Centre));
...
internal void
ArcLineAngles(image_buffer *Buffer, v2 Centre, f32 Radius, f32 Angle1, f32 Angle2, colour Colour)
{
	BEGIN_TIMED_BLOCK;
	DebugClear();
	if(Angle1 > TAU)  Angle1 -= TAU;
	...
	uint NumSegs = (uint)(TotalAngle / TAU_8) % 8;
	++NumSegs; // 1 to 8 rather than 0 to 7
	/* DebugAdd("Angle1: %.4f; Angle2: %.4f\nIncluded Angle: %f\n", Angle1/TAU, Angle2/TAU, TotalAngle); */

	u8 SegDrawn = 0, SegComplete = 0;
	uint Seg = (uint)(Angle1 / TAU_8) % 8;
	uint StartSeg = Seg;
	uint EndSeg = Seg;
	...
	b32 OneSegOnly = StartSeg == EndSeg;
	f32 StartAngle = Angle1 - (f32)StartSeg * TAU_8;
	f32 EndAngle = Angle2 - (f32)EndSeg * TAU_8;
	...
	{
		++LoopCounter;
		f32 PixelAngle = TAU - Angle0(RelPos);
		f32 MirrorPixelAngle = TAU_8 - PixelAngle;
...
#define DRAW_PIXEL(x,y) DEBUGDrawCheckedPixel(Buffer, V2Add(Centre, V2(x, y)), PixelAngle<TAU/16 ? RED : Colour)
		// NOTE: different versions required due to mirroring (inequality signs flipped)
		// TODO: special case one-seg?
		// TODO: this can almost certainly be boiled down
#define DRAWN(x, angle) SegComplete & (1 << x) || (SegDrawn & (1 << x) &&\
				( (x == StartSeg && OneSegOnly  && angle >= StartAngle && angle <= EndAngle) ||\
				  (x == StartSeg && !OneSegOnly && angle >= StartAngle) ||\
				  (x == EndSeg   && !OneSegOnly && angle <= EndAngle) \
				) )
		if(DRAWN(0,       PixelAngle)) DRAW_PIXEL( RelPos.X, -RelPos.Y); /* NEE */
		if(DRAWN(1, MirrorPixelAngle)) DRAW_PIXEL(-RelPos.Y,  RelPos.X); /* NNE */
		if(DRAWN(2,       PixelAngle)) DRAW_PIXEL( RelPos.Y,  RelPos.X); /* NNW */
		if(DRAWN(3, MirrorPixelAngle)) DRAW_PIXEL(-RelPos.X, -RelPos.Y); /* NWW */
           
		if(DRAWN(4,       PixelAngle)) DRAW_PIXEL(-RelPos.X,  RelPos.Y); /* SWW */ 
		if(DRAWN(5, MirrorPixelAngle)) DRAW_PIXEL( RelPos.Y, -RelPos.X); /* SSW */
		if(DRAWN(6,       PixelAngle)) DRAW_PIXEL(-RelPos.Y, -RelPos.X); /* SSE */
		if(DRAWN(7, MirrorPixelAngle)) DRAW_PIXEL( RelPos.X,  RelPos.Y); /* SEE */
#undef DRAW_PIXEL
...
ArcLine(image_buffer *Buffer, v2 Centre, f32 Radius, v2 A, v2 B, colour Colour)
{
	BEGIN_TIMED_BLOCK;
	LOG("");
	// TODO: make all vector-based?
	f32 Angle1 = Angle0(A);
	f32 Angle2 = Angle0(B);
	DebugReplace("A: (%3f, %3f); B: (%3f, %3f)\n", A.X, A.Y, B.X, B.Y);
	v2 SegBoundaries[] =
	{
		V2( 1.f,  0.f), // RIGHT
		V2( 1.f,  1.f), // UP-RIGHT
		...
		V2(-1.f,  0.f), // LEFT
		V2(-1.f, -1.f), // DOWN-LEFT
		V2( 0.f, -1.f), // DOWN
		V2( 1.f, -1.f), // DOWN-RIGHT
	};
	if(Angle1 > TAU)  Angle1 -= TAU;
	if(Angle2 > TAU)  Angle2 -= TAU;
	// NOTE: Only exact pixel widths (for now)
	...
	if(StartSeg == EndSeg)
	{
		SegTypes[StartSeg] = SEG_StartAndEnd;
	}
	else
	{
	...
	while(RelPos.X >= -RelPos.Y)
	{
		++LoopCounter;
		v2 TestV;
		if(LoopCounter == 30 || LoopCounter == 150)
		{
			Shown = 1;
			TestV = V2(-RelPos.X, -RelPos.Y); //3
		}
#define DRAWFULL(seg)	   (SegTypes[seg] == SEG_Full)
#define DRAWSTART(seg)     (SegTypes[seg] == SEG_Start && PerpDot(A, TestV) >= 0 && V2WithinNarrowBoundaries(TestV, A, SegBoundaries[StartSeg+1]))
#define DRAWEND(seg)       (SegTypes[seg] == SEG_End   && PerpDot(B, TestV) <= 0 && V2WithinNarrowBoundaries(TestV, SegBoundaries[EndSeg], B))
#define DRAWSTART_END(seg) (SegTypes[seg] == SEG_StartAndEnd && (\
								(!NumBoundaries && V2WithinNarrowBoundaries(TestV, A, B)) ||\
								(NumBoundaries && !V2WithinNarrowBoundaries(TestV, B, A))))
#define DRAW(seg)	    (SegTypes[seg]) && (DRAWFULL(seg) || DRAWSTART_END(seg) || DRAWSTART(seg) || DRAWEND(seg))
#define DRAW_PIXEL(seg,pix) DEBUGDrawCheckedPixel(Buffer, V2Add(Centre, pix), Colour)
		TestV = V2( RelPos.X,  -RelPos.Y);
		if(DRAW(0)) DRAW_PIXEL(0, TestV); /* NEE */
		TestV = V2(-RelPos.Y,  RelPos.X);
		if(DRAW(1)) DRAW_PIXEL(1, TestV); /* NNE */ // MIRROR
		TestV = V2( RelPos.Y,  RelPos.X);
		if(DRAW(2)) DRAW_PIXEL(2, TestV); /* NNW */
		TestV = V2(-RelPos.X, -RelPos.Y);
		if(DRAW(3)) DRAW_PIXEL(3, TestV); /* NWW */ // MIRROR

		TestV = V2(-RelPos.X,  RelPos.Y);
		if(DRAW(4)) DRAW_PIXEL(4, TestV); /* SWW */ 
		TestV = V2( RelPos.Y, -RelPos.X);
		if(DRAW(5)) DRAW_PIXEL(5, TestV); /* SSW */ // MIRROR
		TestV = V2(-RelPos.Y, -RelPos.X);
		if(DRAW(6)) DRAW_PIXEL(6, TestV); /* SSE */
		TestV = V2( RelPos.X,  RelPos.Y);
		if(DRAW(7)) DRAW_PIXEL(7, TestV); /* SEE */ // MIRROR
#undef DRAW_PIXEL
...
internal void
DrawRectangleLines(image_buffer *Buffer,
					v2 vMin, v2 vMax,
					colour Colour)
{
	BEGIN_TIMED_BLOCK;
