...
Proj(v2 V, v2 W)
{
	...
	v2 Result = V2Mult((Dot(V, W)/V2LenSq(W)), W);
	return Result;
}
...
IntersectSegmentCircle(v2 P, v2 Dir, v2 poFocus, f32 Radius, v2 *Intersection1, v2 *Intersection2)
{
	...
	{ 
		...
		if(t1 < 0.f || V2LenSq(V2Mult(t1, d)) > V2LenSq(Dir))  FirstPoint  = 0;
		if(t2 < 0.f || V2LenSq(V2Mult(t2, d)) > V2LenSq(Dir))  SecondPoint = 0;
...
AABBWidth(aabb A)
{
	Assert(A.MaxX >= A.MinX);
	f32 Result = A.MaxX - A.MinX;
	return Result;
}
internal inline f32
AABBHeight(aabb A)
{
	Assert(A.MaxY >= A.MinY);
	f32 Result = A.MaxY - A.MinY;
	return Result;
}
